...
async function updateCheckoutDetails(userToken: string, data: { basket_id: string, package_id: number, checkout_url: string }) {
	const response = await fetch('https://api.imperfectgamers.org/user/updateCheckoutDetails', {
		method: 'POST',
		headers: {
...
export const action = async ({ request }: ActionFunctionArgs) => {
	...
	if (!cookieHeader) {
		...
		if (!checkout_url){
			throw new Error('User does not have a checkout URL to add to the database.')
		}
		...
		if (checkOutDetails?.status && checkOutDetails.status === 'success' && checkOutDetails.message) {
			return json(
				{
			...
			return json({ error: 'Failed to update checkout details' }, { status: 400 })
		}
		...
		if (error instanceof Error) {
			...
			console.error('[store.session.save.tsx] An unexpected error occurred:', error)
			return json({ error: '[store.session.save.tsx] An unexpected error occurred' }, { status: 500 })
		}
	}
}