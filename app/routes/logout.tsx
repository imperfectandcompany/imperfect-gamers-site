...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    ...
    if (logoutResult.ok) {
      ...
      return json({ success: 'Logout successful. Token was invalid, so session was destroyed.' }, { headers })
    }
  } catch (error) {
  ...
  return json({ error: 'An error occurred during logout. Please try again later.' }, { status: 500 });
};
