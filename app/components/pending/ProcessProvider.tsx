...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'START':
      return { ...state, inProgress: true, progress: 0, message: action.message };
    case 'UPDATE':
...
export const ProcessProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startProcess = useCallback((message: string) => {
    if (!state.inProgress) {
  ...
  return (
    <DispatchContext.Provider value={{ startProcess, processState: state }}>
      {state.inProgress && <MessageContainer message={state.message} />}
      {children}
