...
const ModalHeader: React.FC<ModalHeaderProps> = ({ title, onBack, align = 'center' }) => {
	...
	return (
		<div className="">
			{onBack ? (
				<>
<div className="flex items-center form-header mb-4 " style={{ width: '100%' }}>
    <button className="cursor-pointer flex items-center" onClick={(e) => handleBackClick(e)}>
    <svg
...
<span className="select-none">Back</span>
    ...
    <div className="form-title-modal select-none" style={titleStyle}>{title}</div>
</div>
				</>
			) : (
<div className="flex items-center form-header mb-4 " style={{ width: '100%' }}>
